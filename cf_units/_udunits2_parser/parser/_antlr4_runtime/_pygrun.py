...
def main():
    ...
    if not os.path.exists(lexer_file):
        print(f"[ERROR] Can't find lexer file {lexer_file}!")
        print(os.path.realpath("."))
        exit(1)
    if not os.path.exists(parser_file):
        print(f"[ERROR] Can't find parser file {lexer_file}!")
        print(os.path.realpath("."))
        exit(1)
    ...
    def process(input_stream, class_lexer, class_parser):
        ...
        if hasattr(parser, start_rule):
            ...
            print(
                f"[ERROR] Can't find start rule '{start_rule}' in parser '{parserName}'"
            )
